...
Class News extends Base 
{
	...
	*/
	public function newsList($param)
	{
		...
		$field  = isset($param['field'])  ? $param['field']  : $this->field;//查询字段
		$start  = ((isset($param['page']) ? $param['page']   : $this->page) - 1) * $offset;
		$order  = isset($param['order']) ? $param['order'] : 'id desc'; //排序方式
		$column_id = isset($param['column_id']) ? $param['column_id'] : 1;//栏目id
		//查询是否有二级栏目
	...
	*/
	public function newsDetail($param)
	{
		...
		if($info['is_recommend'] != 0) { // 推荐相关新闻查询
			...
			if(!empty($related_info['related_ids'])) {
		        ...
		        $field = ['id as news_id,title,summary,comments_num,publish_time,source_type,column_ids'];
	...
	public function hotNews($param)
	{
		...
		if(isset($param['min_news_id'])) { // 当前页最大的新闻id ,防止数据重复
			$min_news_id = $param['min_news_id'];
			$where['id'] = ['lt',"$min_news_id"];
		}
