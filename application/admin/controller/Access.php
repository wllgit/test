...
//		 $field = '*';
//		 $where = ['status' => ['<>',0]];
//		 $list  = Db::table('node')->where($where)->field($field)->select();
		if(request()->isAjax()){
            $param = input('');
            ...
            $Result=Db::table('node')->limit($offset, $limit)->where($where)->select();
            ...
            $return['total'] = Db::table('node')->where($where)->count();  //总数据
            $return['rows'] = $Result;
            return json($return);
        }
        return $this->fetch();
...
//            print_r($param);
//            die;
        	$result = Db::table('node')->insert($param);
            if(false === $result){
