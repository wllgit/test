...
class Admin extends Model
{
    ...
    {
        return $this->field('scs_admin.*,scs_role.rolename')
            ->join('scs_role', 'scs_admin.role_id = scs_role.id')
            ->where($where)->limit($offset, $limit)->order('id desc')->select();
    }

    public function getUsers($where = '', $page = 10)
    {
        return $this->field('scs_admin.*,scs_role.rolename')
            ->join('scs_role', 'scs_admin.role_id = scs_role.id')
            ->where($where)->order('scs_admin.id desc')->paginate($page);
    }
